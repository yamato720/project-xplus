...
owner-step8 demo 只完成本地前置验证、完整 hw link 和同步，尚未上板确认 correctness
或性能，因此不晋级标准 bitstream，不更新正式 `source.diff`。
