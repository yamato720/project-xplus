...
X/P vector HBM 读取，但 Matrix_data 读取仍全关闭，且 routed WNS 为 `-0.014 ns`。
用户随后反馈该版成功；本地没有服务器 raw log 或完整计数，因此只作为进入 level 3 的
历史成功边界保留，不把它记为 PCG correctness 结论。
...
服务器验收，level 2 又确认 X/P vector loader；当前同步版已经进入
`loader_drain level=3`，只恢复 Matrix_data ch0/ch15 的 read-drain，仍不恢复 SpMV core。
