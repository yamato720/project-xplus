...
| `cuper-notapa-spmv-u55c-20260703-chisel8-spmvbaseline-demo.xclbin` | no-TAPA Chisel / single SpMV demo | host 或不跑 PCG | `chisel/cuper-spmv8` / `CuperSpmvChisel8` | 独立 Chisel RTL kernel full SpMV owner-step8 phase-1 demo，保持 ABI/HBM mapping 不变，接入 ptr/X/matrix loaders、Chisel Core/Accumulator datapath、scalar `Y_out` writer 和 Status/Metrics writer；当前同步版用 `CUPER_SPMV_CHISEL8_SLIM_DEBUG=1` 隔离重 debug fanout，并把 matrix issue 从 serial source/owner 改为同 owner slot 跨最多 8 source 发射；Vitis link `impl Complete`，DATA/KERNEL/HBM clock 为 `138/500/450 MHz`，routed timing 仍有 setup violation；服务器侧 `CHECK_Y=1` 已通过完整 `thermal2`，完整点 `245.090 ms`、`max_abs_diff=1.63e-6`，但仍比 strip8 `2.71420 ms` 慢约 `90.3x`，不是标准 bitstream |
...
| `cuper-tapa-pcg-fpga-u55c-20260711-demo.xclbin` | TAPA Cuper / FPGA-PCG debug demo | FPGA kernel | `DLC/Cuper-callipepla-pcg/kernels/Cuper.cpp` / `CuperPcgCallipepla` | Callipepla `loader_drain level=2` 定位 artifact；在已通过的 ptr/PE level 1 基线上恢复真实 X/P vector HBM 读取和 vector drain，Matrix_data/SpMV core 仍关闭并继续 fake-ack；请求 DATA/KERNEL/HBM 为 100/500/450 MHz，xclbin metadata 为 100/500/447 MHz；impl complete，但 routed WNS `-0.014 ns`，仅同步用于服务器 debug，不是 timing-clean artifact |
...
no-TAPA Chisel / SpMV drain-probe 实验文件：

```text
cuper-notapa-spmv-u55c-20260703-chisel8-drainprobe-demo.xclbin
```

这版是独立 Vitis RTL kernel `CuperSpmvChisel8`，由 `chisel/cuper-spmv8` 生成
RTL、Vivado 打包 XO、Vitis link 生成 xclbin。它不是
`CuperSpmvServiceOnly` 的 TAPA graph，也还不是完整 SpMV datapath。当前 FSM 做
HBM drain-probe：读取 `ptr[0..7]` 得到每路 matrix beat 数，读取完整 boundary
table，读取 `ceil(Column_num/16)` 个 X packets，再按 `ptr[channel]` drain
`Matrix_data_0..7`。它只写 `Y_out[0]=0`、`Status[0..63]` 和 `Metrics[0..63]`，
用于确认 host ABI、AXI-Lite register map、HBM 端口、读计数和 mmap 返回链路。
...
```text
file: 395bitstream/cuper-notapa-spmv-u55c-20260703-chisel8-drainprobe-demo.xclbin
...
Status: Vitis link `impl Complete`，已同步；服务器侧 no-check 从 `thermal2_n16` 到完整 `thermal2` 全部 `rc=0`，ptr/X/matrix 计数匹配，done mask `0xff`，R/B error mask `0`；`--check-y` 抽样 `rc=3` 为预期失败，不做 SpMV 正确性或性能结论
```

no-check 上板验收只采信 magic、ptr/X/matrix 计数、8 路 done mask 和 response error
mask；`CHECK_Y=1` 仍预期失败，因为 drain-probe 不计算 SpMV。

历史 entry-probe artifact 仍保留在同步目录：
...
Status: entry-probe first-read 历史 artifact，未上板；当前 Chisel8 后续测试应优先使用 drain-probe demo
```

本轮在当前机器尝试复测 entry-probe xclbin 时，XRT 返回 `No such device with index '0'`。
drain-probe xclbin 的服务器侧 no-check 全套 `thermal2` sweep 已通过；当前 Chisel8
测试已转向下面的 full SpMV baseline demo。

no-TAPA Chisel / SpMV baseline / correctness-debug demo 文件：

```text
cuper-notapa-spmv-u55c-20260703-chisel8-spmvbaseline-demo.xclbin
```

这版仍是独立 Vitis RTL kernel `CuperSpmvChisel8`，ABI、AXI-Lite offsets、13 路
`m_axi_*` 端口和 HBM 分配沿用 drain-probe。内部是 full SpMV correctness-debug
baseline：
ptr loader 读取每路 matrix length 和 boundary table，X loader 读取
`ceil(Column_num/16)` 个 packet，8 路 Matrix loader 顺序读各自 HBM，随后接入
...
```text
previous synchronized correctness-debug demo:
...
---
previous synchronized slim/no-debug correctness demo:
file: 395bitstream/cuper-notapa-spmv-u55c-20260703-chisel8-spmvbaseline-demo.xclbin
kernel: CuperSpmvChisel8
...
Status: Vitis link `impl Complete` / `Run completed`，已同步；保持 fmul 7 拍和 fadd 12 拍修复，但用 `CUPER_SPMV_CHISEL8_SLIM_DEBUG=1` 关闭重 debug fanout，`Status[40..61]` / `Metrics[47..63]` ABI 保留但 debug counters 预期为 0；150 MHz DATA timing 仍未收敛，Vitis xclbin info 记录 DATA clock 为 120 MHz；服务器侧 `CHECK_Y=1` 已通过 listed `thermal2*`，完整 `thermal2` 为 `459.425 ms`，远慢于 strip8 `2.71420 ms`，不是标准 bitstream
---
current synchronized owner-step8 phase-1 demo:
file: 395bitstream/cuper-notapa-spmv-u55c-20260703-chisel8-spmvbaseline-demo.xclbin
kernel: CuperSpmvChisel8
...
Status: Vitis link `impl Complete` / `Run completed`，已同步到同一个 demo 槽；保持 ABI/HBM mapping/AXI-Lite offsets/13 路 `m_axi_*` 端口和 scalar `Y_out` writer 不变，仍使用 `CUPER_SPMV_CHISEL8_SLIM_DEBUG=1`；datapath issue 改为每 source 预取一个 pending beat、同 owner slot 跨最多 8 source 发射，X cache 使用 8 份单读/单写 `SyncReadMem`，避免回到旧 64-read-port RTL 爆内存边界；150 MHz DATA timing 仍未收敛，Vitis xclbin info 记录 DATA clock 为 138 MHz；服务器侧 `CHECK_Y=1` 已通过所有 listed `thermal2*`，完整 `thermal2` 为 `245.090 ms`，`max_abs_diff=1.63e-6`，slim debug counters 全 0 为预期；比上一版 slim/no-debug `459.425 ms` 快 `1.88x`，但仍比 strip8 `2.71420 ms` 慢约 `90.3x`，不晋级标准 bitstream
```
...
当前 owner-step8 同步版为了降低布线 fanout，debug counters 槽位保留但预期为 0。
上一版 slim/no-debug demo 已完成板上 correctness，但性能仍远慢于 strip8；当前
owner-step8 demo 已完成板上 correctness 和性能 sweep，完整 `thermal2` 为
`245.090 ms`，比 slim/no-debug 快但仍远慢于 strip8，因此不晋级标准 bitstream，
不更新正式 `source.diff`。下一轮源码已转向 512-bit `Y_out` vector writer，但尚未
覆盖同步本文件中的 owner-step8 demo。

服务器侧反馈目录为 `logs/spmv_chisel8_correctness_debug_hw_20260704_192807/`；该目录
当前未同步到本地仓库，本记录只登记用户提供的结论。反馈显示 ptr/X/matrix decode
和 accumulator accept 链路是活的，但旧 `nonzero_products` 只说明 fmul 输入
...
性能成绩，也不能套用到当前 `09ac7fd6-...` owner-step8 demo。
...
cuper-tapa-spmv-u55c-20260701-ownerbank8-lighttrace-demo.xclbin
```
...
ownerbank8-lighttrace:
  file: 395bitstream/cuper-tapa-spmv-u55c-20260701-ownerbank8-lighttrace-demo.xclbin
...
| 数据集 | original8 FPGA ms | strip8 FPGA ms | lanereal8-scoreboard | lanereal8-scoreboard-donedrain | ownerbank8 |
| --- | ---: | ---: | --- | --- | --- |
| `thermal2_n16` | 0.074269 | 0.079358 | 0.080481, PASS | 0.081832, PASS | PASS |
| `thermal2_n1024` | 0.077615 | 0.083637 | timeout 300s | timeout 300s, rc=124 | timeout 300s |
| `thermal2_n4096` | 0.089127 | 0.087664 | 未继续 | 未继续 | 未继续 |
| `thermal2_n16384` | 0.109745 | 0.124493 | 未继续 | 未继续 | 未继续 |
| `thermal2_n65536` | 0.216074 | 0.222817 | 未继续 | 未继续 | 未继续 |
| `thermal2_n131072` | 0.345646 | 0.365984 | 未继续 | 未继续 | 未继续 |
| `thermal2_n262144` | 0.632533 | 0.622525 | 未继续 | 未继续 | 未继续 |
| `thermal2` | 2.74379 | 2.71420 | 未继续 | 未继续 | 未继续 |
...
output backpressure、write response delay 和小 FIFO 压力。服务器侧上板结果为
`thermal2_n16` PASS，但 `thermal2_n1024` 300s timeout；timeout 采样中
`Status/Metrics` 仍是初始化哨兵，`progress_magic valid=0`，`Y[0..15]` 全 0。
因此它不替代 strip8 的 `2.71420 ms` 实测结论，也不进入 HTML 性能曲线。
已同步的调试版为 `ownerbank8-lighttrace`：保持同一 ABI 和 8-HBM bank mapping，
只在 `Status/Metrics[8..15]` 通过 progress writer 增加 entry、ptr、vector、
...
`CuperPcgCallipepla` `loader_drain level=2` 定位 artifact。它保持 Callipepla
full-PCG 顶层 kernel 名、host 参数顺序、AXI-Lite register offsets、HBM mapping、
Tau/dimension 判断和 probe mode 名称不变。在 level 1 的真实 strip ptr loader、16 路
matrix-length fanout 和 `PE_Param` drain 基础上，恢复真实
`PcgCallipepla_Vector_Loader`：按 command 从 X/P 当前 bank 读取 `double_v8`，转换为
`float_v16` 后由 tail drain 消费。16 路 matrix drain 仍不发 `Matrix_data` request，
SpMV core、accumulator、checker 和 sort 仍未恢复，vector phase 继续 fake-ack。因此
该文件只定位 vector loader/control/finalization 边界，不执行完整 SpMV/PCG datapath，
也不替换当前标准 `cuper-tapa-pcg-fpga-u55c-20260525.xclbin`。

当前 demo xclbin UUID 为 `04f7d703-2011-fd0a-6c43-aa158ddfbd12`，SHA256 为
`a9844caf9e07084e8d518a6e4c5a2c6e36a652aff3f39af978b4a78ef35446a1`，
`.xclbin.info` SHA256 为
`65f0ff445c74aab369a7294d8e44a7f2a32e5aea21f73b2f2df991b2dad81975`。
请求/约束 DATA/KERNEL/HBM clock 为 `100/500/450 MHz`，最终 xclbin metadata 为
`100/500/447 MHz`。Vitis link 退出码为 0，`impl Complete`，VPL 和 POST-VPL 均为
0 errors，总耗时 `1h53m45s`；但 routed timing 未完全收敛：WNS `-0.014 ns`、TNS
`-0.053 ns`、setup failing endpoints `10`、WHS `0.009 ns`、THS `0.000 ns`。失败路径
位于平台 `hmss_0` 的 450 MHz HBM inter-SLR response pipeline，不在 100 MHz DATA
或 500 MHz KERNEL clock domain。构建日志为
`logs/cuper_tapa_pcg_callipepla_loader_monitor_level2_hw_20260711_134233.log`，构建目录为
`cuper-tapa-pcg-callipepla-loader-monitor-level2-xo-build/`。该 artifact 按用户要求直接
同步用于服务器侧 debug，明确不是 timing-clean 版本。

level 2 software/TAPA simulation 已覆盖 `thermal2_n16 MAX_ITERS=0/1/10`：vector
commands/rounds/HBM words 为 `2/1/2`、`3/2/4`、`12/11/22`；ptr commands 为
`2/3/12`，PE rounds 为 `1/2/11`，ptr HBM words 为 `48/80/368`。所有点均为
`event=99`、full=0、flags=`0x7e`、三类 event drop 为 0。level 1、mode-2 cmd-drain
和无 probe full graph 的软件回归也保持原结果。当前 level-2 artifact 尚无服务器
raw log 或上板结论，不进入 PCG correctness、分段时间或性能图表，也不更新正式
`source.diff`。

上一同步的 level-1 artifact UUID 为 `fdbc2e10-20ea-8e78-6b3c-72a01803cde1`，SHA256
为 `0e0e7e832b24afff008665f45ea0020c9cb121cfcf3c2c0f1cf20ea13ee76fe0`。它在
`100/500/450 MHz` 下 routed timing clean，服务器侧已按约定的 level-1 验收口径通过：
`event=99`、各项计数符合预期、`command_full=0`、三类 event drop 为 0，
controller/ack/ptr/PE done flag 全部置位，XRT error 为空且 CU 回到 IDLE。本地没有
服务器 raw log，本记录继续按用户提供的结论作为历史基线保留。
...
`840018ee2c1cb30e7e8ed6f5f6c815e9abf830f9e060de850bfc6aeecb17198c`。它已从同步目录
删除，但服务器侧握手测试结论继续作为历史基线保留。

上一版 command/result 顺序修复 UUID `d46c3285-6cc2-1b02-9350-1ad3dadb5c56` 在
服务器侧 `thermal2_n16 MAX_ITERS=0` timeout，最后可见 `Status[52]=60`。后续 XO
审查确认事务内部的细粒度 Status 写被 HLS 合并或消除，因此 `60` 不能证明 command
未接受；该结论只作为历史定位边界，当前同步文件已由独立 monitor 版覆盖。

上一版 thin-status cmd-drain UUID `ad7b2a61-23d4-5c05-360d-acb2ee604830` 在服务器侧
`thermal2_n16` 最小 smoke 稳定停在 `Status[52]=14`。旧 controller schedule/RTL
显示 valid 分支在发 vector command 前先依赖 result-empty，形成真实调度死锁；该旧
artifact 已被当前顺序修复版覆盖。更早的多槽 checkpoint cmd-drain UUID
`ea2f5c5a-f0f9-c536-8caf-7faa82aa4107` 停在 `Status[52]=11`，两者只作为历史定位边界。
...
2026-07-11 服务器侧反馈确认上一 cmd-drain artifact 已完成 demo-only 握手验证：
...
artifact，不做 PCG diff/性能结论。上一同步版随后进入 `loader_drain level=1` 并已通过
服务器验收；当前同步版已经进入 `loader_drain level=2`，仍不恢复 Matrix_data 或
SpMV core。
...
后续 Callipepla probe demo 替换；旧 demo-only 结论只作为历史记录保留，不再对应
同步目录中的当前 full-PCG demo 文件。
