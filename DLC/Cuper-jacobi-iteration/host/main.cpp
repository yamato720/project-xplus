...
int RunSpmvServiceOnly(const std::string& bitstream,
    ...
    Create_SpElement_list_for_all_channels_lane_static_real_batch(
                                           m,
                                           Slice_SIZE,
                                           BATCH_SIZE,
                                           sliceMatrix,
...
#elif defined(JACOBI_SPMV_COMPACT_PE)
    vector<INDEX_TYPE> Matrix_len_per_hbm;
    vector<INDEX_TYPE> SpElement_list_ptr_per_hbm;
    ...
    cout << "[spmv-only-lane-static-real] enabled=1"
         << " original_read_beats=" << original_read_beats
         << " lane_static_read_beats=" << stripped_matrix_len_total
    ...
    cout << "[spmv-only-strip-padding] enabled=1"
         << " original_read_beats=" << original_read_beats
         << " stripped_read_beats=" << stripped_matrix_len_total
         << " saved_beats=" << (original_read_beats - stripped_matrix_len_total)
         << " saved_pct="
         << (original_read_beats == 0
                 ? 0.0
         ...
         << endl;
#else
    cout << "[spmv-only-strip-padding] enabled=0" << endl;
#endif
