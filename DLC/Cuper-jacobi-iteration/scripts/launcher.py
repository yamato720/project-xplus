    ...
    run_script_path.write_text(
        "\n".join(
            [
                ...
                *(["export JACOBI_TRACE_ISOTOPE=1"] if os.environ.get("JACOBI_TRACE_ISOTOPE") not in {None, "", "0"} else []),
                *(["export JACOBI_TRACE_LIGHT=1"] if os.environ.get("JACOBI_TRACE_LIGHT") not in {None, "", "0"} else []),
                *(["export JACOBI_WIDE_HBM=1"] if os.environ.get("JACOBI_WIDE_HBM") not in {None, "", "0"} else []),
                *(["export JACOBI_HBM_CHANNELS=" + shell_quote(os.environ["JACOBI_HBM_CHANNELS"])] if os.environ.get("JACOBI_HBM_CHANNELS") else []),
                *(["export JACOBI_BLOCKING_ENTRY_PROBE=1"] if os.environ.get("JACOBI_BLOCKING_ENTRY_PROBE") not in {None, "", "0"} else []),
                *(["export CLOCK_PERIOD=" + shell_quote(os.environ["CLOCK_PERIOD"])] if os.environ.get("CLOCK_PERIOD") else []),
                ...
                *(["export JACOBI_SPMV_ENTRY_PROBE=1"] if os.environ.get("JACOBI_SPMV_ENTRY_PROBE") not in {None, "", "0"} else []),
                ...
                *(["export JACOBI_TAPA_ENABLE_SYNTH_UTIL=" + shell_quote(os.environ["JACOBI_TAPA_ENABLE_SYNTH_UTIL"])] if os.environ.get("JACOBI_TAPA_ENABLE_SYNTH_UTIL") not in {None, ""} else []),
    ...
    while True:
        ...
        if answer == "2":
            ...
            command = (
                ...
                f"unset BITFILE && {shell_quote(build_dir / 'cuper_jacobi_host')} {shell_quote(dataset.path)}"
            )
        ...
        if answer == "4":
            ...
            continue
        if answer == "5":
            rc = start_hw_tmux(root, "cuper_jacobi_iteration_hw_build", force=False)
            print_result(rc)
            return rc
        if answer == "6":
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Cuper Jacobi iteration build/run launcher.")
    ...
    hw_tmux.add_argument("--session", default="cuper_jacobi_iteration_hw_build")
...
def main() -> int:
    ...
    if args.command == "run-sw":
        ...
        command = (
            "source scripts/env_u55c.sh && "
            f"unset BITFILE && {shell_quote(build_dir / 'cuper_jacobi_host')} {shell_quote(matrix)}"
        )
        return run_shell(command, root)
    ...
    while True:
        print()
        print("Cuper Jacobi Iteration Launcher")
