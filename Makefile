...
cuper-jacobi-run-pack-profile:
	@$(MAKE) -C "$(CUPER_JACOBI_DIR)" run-pack-profile BUILD_DIR="$(CUPER_JACOBI_BUILD_DIR)" MATRIX="$(abspath $(or $(MATRIX),data/suitesparse/Schmid/csr/thermal2_n1024))" $(if $(HBM),HBM="$(HBM)") $(if $(TOP_BATCHES),TOP_BATCHES="$(TOP_BATCHES)") $(if $(DUMP_BATCH),DUMP_BATCH="$(DUMP_BATCH)") $(if $(DUMP_BEATS),DUMP_BEATS="$(DUMP_BEATS)") $(if $(CSV),CSV="$(CSV)") $(if $(DROP_DIAG),DROP_DIAG="$(DROP_DIAG)")
